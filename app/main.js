(function($, window) {
	...
	/**
	 * Compose the styles select
	 * @param {Array} data
	 * @private
	 */
	function _composeOptions(data) {
		select.html('');
	 ...
	 * @private
	 */
	function _sendInputOption(data) {
		select.html('');
		
		if(!data) {
			$.ajax({
				url: BASE_URL+'/carOptions',
				type: 'GET'
			}).then(_composeOptions, _logError);
		}
		else {
			_composeOptions(data);
		}
	}
	...
	function _init() {
		...
		styleForm.on('submit', _submitForm2);
	}
